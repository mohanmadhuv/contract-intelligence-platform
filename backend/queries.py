...
PostgreSQL dialect for Render deployment.
"""
...
def _yf(y0, y1):
    return f"contract_date IS NOT NULL AND CAST(contract_value AS FLOAT) > 0 AND EXTRACT(YEAR FROM contract_date::date) BETWEEN {y0} AND {y1}"


def get_overview(y0=2015, y1=2025):
    r = query(f"""SELECT COUNT(*) AS total_contracts,
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS total_spend_cad,
        COUNT(DISTINCT vendor_name) AS unique_vendors,
        COUNT(DISTINCT owner_org) AS departments,
        MIN(EXTRACT(YEAR FROM contract_date::date)) AS year_min,
        MAX(EXTRACT(YEAR FROM contract_date::date)) AS year_max,
        ROUND(AVG(CAST(contract_value AS FLOAT)), 0) AS avg_contract_value,
        ROUND(COUNT(*) FILTER (WHERE LOWER(solicitation_procedure) LIKE '%non-comp%'
            OR LOWER(solicitation_procedure) LIKE '%sole%'
            OR LOWER(solicitation_procedure) LIKE '%acan%') * 100.0 / NULLIF(COUNT(*), 0), 2) AS sole_source_pct
...
def get_what_is_bought(y0=2015, y1=2025):
    commodity = query(f"""SELECT COALESCE(NULLIF(TRIM(commodity_type), ''), 'Unknown') AS category,
        COUNT(*) AS contracts, ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend,
...
def get_spend_trend(category=None, y0=2015, y1=2025):
    ...
    return query(f"""WITH y AS (
        SELECT EXTRACT(YEAR FROM contract_date::date) AS year, COUNT(*) AS contracts,
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend, ROUND(AVG(CAST(contract_value AS FLOAT)), 0) AS avg_value
        FROM {_t()} WHERE {_yf(y0, y1)} {cat} GROUP BY 1)
        SELECT *, ROUND((spend - LAG(spend) OVER(ORDER BY year)) * 100.0 /
            NULLIF(LAG(spend) OVER(ORDER BY year), 0), 1) AS spend_yoy_pct
...
def get_growth_ranking(y0=2015, y1=2025):
    return query(f"""WITH first_last AS (
        ...
        SUM(CASE WHEN EXTRACT(YEAR FROM contract_date::date) = {y0} THEN CAST(contract_value AS FLOAT) ELSE 0 END) AS spend_start,
        SUM(CASE WHEN EXTRACT(YEAR FROM contract_date::date) = {y1} THEN CAST(contract_value AS FLOAT) ELSE 0 END) AS spend_end
        FROM {_t()} WHERE {_yf(y0, y1)} GROUP BY 1)
        SELECT category, ROUND(spend_start, 0) AS spend_start, ROUND(spend_end, 0) AS spend_end,
        ROUND((spend_end - spend_start) * 100.0 / NULLIF(spend_start, 0), 1) AS growth_pct
        FROM first_last WHERE spend_start > 0 ORDER BY growth_pct DESC LIMIT 15""")


def get_volume(y0=2015, y1=2025, category=None):
    cat = f"AND TRIM(commodity_type) = '{category}'" if category else ""
    return query(f"""SELECT EXTRACT(YEAR FROM contract_date::date) AS year,
...
def get_unit_cost(y0=2015, y1=2025, category=None):
    ...
    yearly = query(f"""WITH y AS (
        SELECT EXTRACT(YEAR FROM contract_date::date) AS year,
        ROUND(AVG(CAST(contract_value AS FLOAT)), 0) AS avg_value,
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS total_spend, COUNT(*) AS contracts
        FROM {_t()} WHERE {_yf(y0, y1)} {cat} GROUP BY 1)
        SELECT *, ROUND((avg_value - LAG(avg_value) OVER(ORDER BY year)) * 100.0 /
            NULLIF(LAG(avg_value) OVER(ORDER BY year), 0), 1) AS avg_value_yoy_pct
        FROM y ORDER BY year""")
...
def get_amendments(y0=2015, y1=2025, category=None):
    cat = f"AND TRIM(commodity_type) = '{category}'" if category else ""
    yearly = query(f"""SELECT EXTRACT(YEAR FROM contract_date::date) AS year,
        COUNT(*) FILTER (WHERE original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0) AS amended_contracts,
        ROUND(AVG(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) END), 3) AS avg_inflation_ratio,
        ROUND(SUM(CAST(contract_value AS FLOAT) - COALESCE(CAST(original_value AS FLOAT), CAST(contract_value AS FLOAT))), 0) AS total_amendment_value
...
def get_concentration(y0=2015, y1=2025, category=None):
    cat = f"AND TRIM(commodity_type) = '{category}'" if category else ""
    top_vendors = query(f"""WITH t AS (SELECT SUM(CAST(contract_value AS FLOAT)) AS gt FROM {_t()} WHERE {_yf(y0, y1)} {cat})
        SELECT vendor_name, COUNT(*) AS contracts, ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend,
        ROUND(SUM(CAST(contract_value AS FLOAT)) * 100.0 / (SELECT gt FROM t), 2) AS market_share_pct
        FROM {_t()} WHERE {_yf(y0, y1)} {cat}
        GROUP BY vendor_name ORDER BY spend DESC LIMIT 15""")
    hhi_trend = query(f"""WITH yearly_vendor_spend AS (
        SELECT EXTRACT(YEAR FROM contract_date::date) AS year, vendor_name,
        SUM(CAST(contract_value AS FLOAT)) AS spend
...
def get_less_for_more(y0=2015, y1=2025):
    rows = query(f"""WITH base AS (
        ...
        SUM(CAST(contract_value AS FLOAT)) AS spend, COUNT(*) AS contracts,
        AVG(CAST(contract_value AS FLOAT)) AS avg_value,
        AVG(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) END) AS inflation_ratio,
        COUNT(*) FILTER (WHERE LOWER(solicitation_procedure) LIKE '%non-comp%'
            OR LOWER(solicitation_procedure) LIKE '%sole%'
        ...
        ROUND((s1 - s0) * 100.0 / NULLIF(s0, 0), 1) AS spend_growth_pct,
        ROUND((c1 - c0) * 100.0 / NULLIF(c0, 0), 1) AS unit_cost_growth_pct,
...
def get_topline_trend(y0=2015, y1=2025):
    return query(f"""SELECT EXTRACT(YEAR FROM contract_date::date) AS fiscal_year,
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS total_spend,
        COUNT(*) AS contract_count,
        ROUND(AVG(CAST(contract_value AS FLOAT)), 0) AS avg_contract_value,
        ROUND(AVG(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) END), 4) AS avg_amendment
...
def get_category_summaries(y0=2015, y1=2025):
    rows = query(f"""WITH yearly AS (
        ...
        EXTRACT(YEAR FROM contract_date::date) AS year,
        SUM(CAST(contract_value AS FLOAT)) AS spend, COUNT(*) AS cnt, AVG(CAST(contract_value AS FLOAT)) AS avg_val,
        AVG(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) END) AS amend_ratio
        FROM {_t()} WHERE {_yf(y0, y1)} GROUP BY 1, 2),
        hhi_yearly AS (
        SELECT code, year,
        SUM(POW(vendor_spend / NULLIF(total_spend, 0), 2)) AS hhi
        FROM (
            SELECT COALESCE(NULLIF(TRIM(economic_object_code), ''), 'Unknown') AS code,
            EXTRACT(YEAR FROM contract_date::date) AS year,
            vendor_name, SUM(CAST(contract_value AS FLOAT)) AS vendor_spend,
            SUM(SUM(CAST(contract_value AS FLOAT))) OVER(PARTITION BY COALESCE(NULLIF(TRIM(economic_object_code), ''), 'Unknown'), EXTRACT(YEAR FROM contract_date::date)) AS total_spend
        ...
        ROUND(POW(GREATEST(spend1 / NULLIF(spend0, 0), 0.001), 1.0 / NULLIF(n - 1, 0)) - 1, 4) AS cagr_spend,
        ROUND(POW(GREATEST(cnt1::float / NULLIF(cnt0::float, 0), 0.001), 1.0 / NULLIF(n - 1, 0)) - 1, 4) AS cagr_volume,
        ROUND(POW(GREATEST(avg1 / NULLIF(avg0, 0), 0.001), 1.0 / NULLIF(n - 1, 0)) - 1, 4) AS cagr_unit,
...
def get_contracts_table(search=None, fy=None, eoc=None, sort_key="amendment_ratio", sort_dir="desc", limit=80):
    ...
    if fy:
        filters.append(f"EXTRACT(YEAR FROM contract_date::date) = {int(fy)}")
    if eoc:
    ...
    allowed = {
        ...
        "total_value": "CAST(contract_value AS FLOAT)",
        "original_value": "COALESCE(CAST(original_value AS FLOAT), CAST(contract_value AS FLOAT))",
        "vendor": "vendor_name",
    }
    sk = allowed.get(sort_key, "amr")
    sd = "ASC" if sort_dir == "asc" else "DESC"
    return query(f"""SELECT reference_number,
        EXTRACT(YEAR FROM contract_date::date) AS fiscal_year,
        ...
        ROUND(COALESCE(CAST(original_value AS FLOAT), CAST(contract_value AS FLOAT)), 0) AS original_value,
        ROUND(CAST(contract_value AS FLOAT), 0) AS total_value,
        ROUND(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) ELSE 1 END, 3) AS amendment_ratio,
        CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) ELSE 1 END AS amr
...
def get_department_spend(y0=2015, y1=2025):
    return query(f"""SELECT COALESCE(owner_org, 'Unknown') AS code,
        ...
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend
...
def get_watchlist(y0=2015, y1=2025):
    return query(f"""SELECT reference_number,
        ...
        ROUND(COALESCE(CAST(original_value AS FLOAT), CAST(contract_value AS FLOAT)), 0) AS original_value,
        ROUND(CAST(contract_value AS FLOAT), 0) AS total_value,
        ROUND(CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0), 3) AS amendment_ratio
        FROM {_t()} WHERE {_yf(y0, y1)}
        AND original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
        AND CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) > 1.15
        ORDER BY CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) DESC LIMIT 30""")


def get_category_year_rows(eoc, y0=2015, y1=2025):
    rows = query(f"""WITH yearly AS (
        SELECT EXTRACT(YEAR FROM contract_date::date) AS year,
        ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend, COUNT(*) AS contracts,
        ROUND(AVG(CAST(contract_value AS FLOAT)), 0) AS avg_value,
        ROUND(AVG(CASE WHEN original_value IS NOT NULL AND CAST(original_value AS FLOAT) > 0
            THEN CAST(contract_value AS FLOAT) / NULLIF(CAST(original_value AS FLOAT), 0) END), 3) AS amendment_ratio
        FROM {_t()} WHERE {_yf(y0, y1)} AND TRIM(economic_object_code) = '{eoc}'
        GROUP BY 1),
        hhi AS (
        SELECT year, SUM(POW(vendor_spend / NULLIF(total_spend, 0), 2)) AS hhi
        FROM (
            SELECT EXTRACT(YEAR FROM contract_date::date) AS year, vendor_name,
            SUM(CAST(contract_value AS FLOAT)) AS vendor_spend,
            SUM(SUM(CAST(contract_value AS FLOAT))) OVER(PARTITION BY EXTRACT(YEAR FROM contract_date::date)) AS total_spend
            FROM {_t()} WHERE {_yf(y0, y1)} AND TRIM(economic_object_code) = '{eoc}'
            GROUP BY 1, 2
        ) sub GROUP BY 1)
...
def get_vendor_shares(eoc, y0=2015, y1=2025):
    return query(f"""WITH total AS (
        SELECT SUM(CAST(contract_value AS FLOAT)) AS t
        FROM {_t()} WHERE {_yf(y0, y1)} AND TRIM(economic_object_code) = '{eoc}')
        SELECT vendor_name AS vendor, ROUND(SUM(CAST(contract_value AS FLOAT)), 0) AS spend,
        ROUND(SUM(CAST(contract_value AS FLOAT)) / NULLIF((SELECT t FROM total), 0), 4) AS share
        FROM {_t()} WHERE {_yf(y0, y1)} AND TRIM(economic_object_code) = '{eoc}'
...
def get_geography(y0=2015, y1=2025):
    rows = query(f"""WITH provs AS (
        ...
        CAST(contract_value AS FLOAT) AS val
        FROM {_t()} WHERE {_yf(y0, y1)}
